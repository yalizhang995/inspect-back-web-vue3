<template>
    <div class="app-container">
        ...
        <div class="bigWrapBox" v-if="upHeightType">
            ...
            <div class="tabBox">
                ...
                >
                    ...
                    <el-table-column prop="title" label="问题地址" width="180" show-overflow-tooltip>
                        <template slot-scope="scope">
         ...
         <el-tabs v-model="activeName" @tab-click="handleClick" v-if="!upHeightType">
            ...
            <el-tab-pane label="待审核" name="1">
                ...
                >
                    ...
                    <el-form-item label="问题类型" prop="typeIn">
                    ...
                    <el-form-item label="资源类别" prop="checkUrlType">
                        <el-select v-model="queryParams.checkUrlType" placeholder="请选择资源类别" style="width:300px" clearable >
                            <el-option
                    ...
                    <el-form-item label="审核角色" prop="auditType">
                        <el-select v-model="queryParams.auditType" placeholder="请选择审核角色" style="width:300px" clearable >
                            <el-option
                    ...
                    <el-form-item label="标签分类" prop="zkTypeChild">
                        <el-select v-model="queryParams.zkTypeChild" filterable placeholder="请选择标签分类" style="width:300px" clearable>
                            <el-option-group
                    ...
                    <el-form-item label="依据" prop="accId">
                        <el-input v-model="queryParams.accId" placeholder="请输入依据ID" clearable style="width:300px" @keyup.enter.native="handleQuery"  @clear="queryParams.accId=null" />
                    </el-form-item>
                    <el-form-item label="AI标记" prop="showStateAi">
                        <el-select v-model="queryParams.showStateAiIn" clearable multiple collapse-tags placeholder="请选择AI标记" style="width:300px">
                            <el-option
        ...
        <!-- 批量显示、预警 -->
        <el-dialog title="操作类型" :visible.sync="open7" width="600px" append-to-body :close-on-click-modal="false" :close-on-press-escape="false">
            <el-form ref="form7" :model="form7" :rules="rules7" label-width="120px">
                <el-form-item label="操作类型" prop="showState">
                    <el-radio-group v-model="form7.showState">
...
import Template from '../../biz/wrongWords/template'
...
export default {
    ...
    methods: {
        ...
        // 问题类型
        getWtlxList(val){
            getPrivacyBreachMap({}).then((res)=>{
                this.wtlxList = [];
                for(var i in res.data){
                    this.wtlxList.push(res.data[i])
                }
                console.log(this.wtlxList,'问题类型数据')
                // this.wtlxList = this.wtlxListAll.slice(0,5);
            })
        },
        // 标签分类
        getErrorTypeFn(){
        ...
        handleClick(tab, event) {
            ...
            if(tab.name == '1'){
                ...
                if((this.queryParams.treeInfo.psCode != null && this.queryParams.treeInfo.psCode != '') || this.queryParams.siteCodeMedia != null || this.queryParams.idStr != null){
                    this.getList();                
                }else{
        ...
        handleQuery() {
            ...
            if((this.queryParams.treeInfo.psCode != null && this.queryParams.treeInfo.psCode != '') || this.queryParams.siteCodeMedia != null || this.queryParams.idStr != null){
                ...
                this.$modal.msgWarning("站点信息/新媒体标识码/数据ID请至少输入其中一项");                
            }
        },
        /** 重置按钮操作 */
        resetQuery() {
                ...
                if (valid) {
                    ...
                    if(this.form4.type == 2){ // 大批量
                        ...
                        if(this.exportSiteName != null && this.exportSiteName_ != null){
                            fileName = this.exportSiteName_ + '-隐私泄露数据-'+this.formatDate(new Date());
                        }else{
                            if(this.exportSiteName != null){
                                fileName = this.exportSiteName + '-隐私泄露数据-'+this.formatDate(new Date());
                            }else if(this.exportSiteName_ != null){
                                fileName = this.exportSiteName_ + '-隐私泄露数据-'+this.formatDate(new Date());
                            }else{
                                fileName = '隐私泄露数据-'+this.formatDate(new Date());
                            }
                        }
                        console.log(fileName,'fileName')
                        this.download(
                            "biz/mediaPrivacyLeaks/export",this.queryParams,
                            `${fileName}.xlsx`
                        ).then((res)=>{
                            this.open4 = false;
                        });
            ...
            if(this.subFileList.length > 0){
                ...
                if(this.fileType == 1){ // 审核导入
                    this.subFileList.forEach(f => {
                        ...
                        formData.append("taskType", 107);
                    });
                }else if(this.fileType == 2){ // 整改导入
                    this.subFileList.forEach(f => {
                        console.log(f,'f')
                        formData.append("file", f.raw);
                        formData.append("taskType", 108);
                    });
                }
        ...
        handleDown(){
            if(this.fileType == 1){ // 审核
                let fileName = '新媒体隐私泄露审核模板';  
                this.download2(
                    "common/download/downloadExcelTpl?type=107",'',
                    `${fileName}.xlsx`
                ).then((res)=>{});
                              
            }else if(this.fileType == 2){ // 整改
                let fileName = '新媒体隐私泄露整改模板';
                this.download2(
                    "common/download/downloadExcelTpl?type=108",'',
                    `${fileName}.xlsx`
                ).then((res)=>{});
            }
        ...
        submitForm7() {
            this.$refs["form7"].validate(valid => {
                if (valid) {
                    let data = {
                        ids: this.ids,
                        showState: this.form7.showState
                    }
                    editBatch(data).then((res)=>{
                        this.$modal.msgSuccess("操作成功");  
                        this.inspSiteInfoList = this.inspSiteInfoList.filter(
