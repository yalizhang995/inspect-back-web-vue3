    ...
    methods: {
                ...
                if (valid) {
                    ...
                    if(this.form4.type == 2){ // 大批量
                        ...
                        if(this.exportSiteName != null && this.exportSiteName_ != null){
                            fileName = this.exportSiteName_ + '-敏感信息数据-'+this.formatDate(new Date());
                        }else{
                            if(this.exportSiteName != null){
                                fileName = this.exportSiteName + '-敏感信息数据-'+this.formatDate(new Date());
                            }else if(this.exportSiteName_ != null){
                                fileName = this.exportSiteName_ + '-敏感信息数据-'+this.formatDate(new Date());
                            }else{
                                fileName = '敏感信息数据-'+this.formatDate(new Date());
                            }
                        }
                        console.log(fileName,'fileName')
                        this.download(
                            "biz/mediaSensitiveWords/export",this.queryParams,
                            `${fileName}.xlsx`
                        ).then((res)=>{
                            this.open4 = false;
                        });
            ...
            if(this.subFileList.length > 0){
                ...
                if(this.fileType == 1){ // 审核导入
                    this.subFileList.forEach(f => {
                        ...
                        formData.append("taskType", 104);
                    });
                }else if(this.fileType == 2){ // 整改导入
                    this.subFileList.forEach(f => {
                        console.log(f,'f')
                        formData.append("file", f.raw);
                        formData.append("taskType", 105);
                    });
                }
        ...
        handleDown(){
            if(this.fileType == 1){ // 审核
                let fileName = '新媒体敏感信息审核模板';  
                this.download2(
                    "common/download/downloadExcelTpl?type=104",'',
                    `${fileName}.xlsx`
                ).then((res)=>{});
                              
            }else if(this.fileType == 2){ // 整改
                let fileName = '新媒体敏感信息整改模板';
                this.download2(
                    "common/download/downloadExcelTpl?type=105",'',
                    `${fileName}.xlsx`
                ).then((res)=>{});
            }
