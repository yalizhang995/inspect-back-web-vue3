         ...
         <el-tabs v-model="activeName" @tab-click="handleClick" v-if="!upHeightType">
            ...
            <el-tab-pane label="已审核错误" name="3">
                ...
                >
                    ...
                    <el-table-column prop="title" label="附件地址" width="100" align="center" show-overflow-tooltip>
                        <template slot-scope="scope">
                            <!-- <el-tooltip class="item" effect="dark" :content="scope.row.title" placement="top"> -->
                                <!-- <span class="dz" @click="dzChange(scope.row.url)">{{ scope.row.url}}</span> -->
                                <span class="dz" @click="dzChange(scope.row.url)"><i class="el-icon-paperclip"></i></span>
                                <!-- el-icon-paperclip -->
                            <!-- </el-tooltip> -->
                        </template>
                    </el-table-column>
            ...
            listInspSiteInfo(this.queryParams).then(response => {
                ...
                for(var i in response.rows){
                    ...
                    if(response.rows[i].fromTitle == null || response.rows[i].fromTitle == ''){
                        if(response.rows[i].fromUrl == null || response.rows[i].fromUrl == ''){
