    ...
    <div class="app-container errorsPage" >
        ...
        <div>
            ...
            >
                ...
                <el-table-column prop="pid" label="主任务ID" min-width="220" ></el-table-column>
                <el-table-column prop="idStr" label="子任务ID" min-width="220" ></el-table-column>
...
export default {
    ...
    beforeRouteLeave(to, from, next) {
        ...
        this.typeVal = false;
        // 确保调用next()以便路由继续跳转
        sessionStorage.removeItem('childreId')
        next();

    },
    created() {
        if(sessionStorage.getItem('childreId')){
            this.queryParams.pid = sessionStorage.getItem('childreId');
            this.getList();
        }
    },
     beforeRouteLeave(to, from, next) {
    ...
    methods: {
        ...
        handleInfo(row, typeVal, reviewStateVal){
            // this.$router.push()
            sessionStorage.setItem('taskChildrenObj', JSON.stringify(row)) // 对象
            sessionStorage.setItem('numType', typeVal); // 详情tab
            sessionStorage.setItem('goInfoType', 2); // 主子任务区分
