         ...
         <el-tabs v-model="activeName" @tab-click="handleClick" v-if="!upHeightType">
            ...
            <el-tab-pane label="已审核错误" name="3">
                ...
                >
                    ...
                    <el-table-column prop="suggest" label="推荐词" width="200">
                        <template slot-scope="scope">
                                <!-- <el-tooltip class="item" effect="dark" :content="scope.row.suggest" placement="top"> -->
                                <span>{{scope.row.suggest}}</span>
                            <!-- </el-tooltip> -->
    ...
    methods: {
            ...
            this.$refs["form3"].validate(valid => {
                if (valid) {
                    ...
                    if(this.form3.type == 2){ // 大批量
                        ...
                        this.download(
                            "biz/sensitiveWords/export",this.queryParams,
                            `${fileName}.xlsx`
                        ).then((res)=>{
                            this.open3 = false;
                        });
            ...
            if(this.subFileList.length > 0){
                ...
                if(this.fileType == 1){ // 审核导入
                    this.subFileList.forEach(f => {
                        ...
                        formData.append("taskType", 4);
                    });
                }else if(this.fileType == 2){ // 整改导入
                    this.subFileList.forEach(f => {
                        console.log(f,'f')
                        formData.append("file", f.raw);
                        formData.append("taskType", 5);
                    });
                }
        ...
        handleDown(){
            if(this.fileType == 1){ // 审核
                let fileName = '网站敏感信息审核模板';  
                this.download2(
                    "common/download/downloadExcelTpl?type=4",'',
                    `${fileName}.xlsx`
                ).then((res)=>{});
                              
            }else if(this.fileType == 2){ // 整改
                let fileName = '网站敏感信息整改模板';
                this.download2(
                    "common/download/downloadExcelTpl?type=5",'',
                    `${fileName}.xlsx`
                ).then((res)=>{});
            }
