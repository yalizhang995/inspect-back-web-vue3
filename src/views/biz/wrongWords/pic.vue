         ...
         <el-tabs v-model="activeName" @tab-click="handleClick" v-if="!upHeightType">
                ...
                >
                    <el-form-item label="主管单位" prop="">
                        <el-autocomplete
                            ...
                            @clear="queryParams.treeInfo.psCode = null"
                        ></el-autocomplete>
                    </el-form-item> 
                    ...
                    <!-- <el-form-item label="订单批次" prop="severity">
                        <el-cascader
                            ...
                            :options="pcList"
                            @change="pcChange"
                            style="width:380px;margin:0 10px 0 0" :popper-append-to-body="false"  popper-class="my-select"
                            >
                    ...
                    <el-form-item label="订单" prop="">
                        <el-select v-model="orderId" filterable remote placeholder="请选择订单"  @change="ddChange" :remote-method="remoteMethod" clearable style="width:310px">
                            <el-option
                    ...
                    <el-form-item label="订单批次" prop="">
                        <el-select v-model="batchId" filterable remote placeholder="请选择订单批次" :remote-method="remoteMethod2" clearable style="width:310px">
                            <el-option
                    ...
                    <el-table-column prop="sentence" label="所在上下文" width="240">
                        <template slot-scope="scope">
                            <!-- <el-tooltip  placement="top">
                                <div slot="content">
                                    {{scope.row.sentence.split(scope.row.word)[0]}}
                                    <span style="color:red">{{scope.row.word}}</span>
                                    {{scope.row.sentence.split(scope.row.word)[1]}}
                                </div>
                                <span>{{scope.row.sentence}}</span>
                            </el-tooltip> -->
                             <!-- <span>
                                <span>{{scope.row.sentence.split(scope.row.word)[0]}}</span>
                                <span style="color:red">{{scope.row.word}}</span>
                                <span>{{scope.row.sentence.split(scope.row.word)[1]}}</span>
                            </span> -->
                            <!-- <el-button type="text" class="kong" @click="aa=true">asd</el-button> -->
                            <el-popover
                                placement="bottom"
                                width="400"
                                v-model="scope.row.sxwType"
                                >
                                <span class="sxwClass">
                            ...
                            <!-- <span v-if="scope.row.sentence" class="sxwClass" @click="scope.row.sxwType = !scope.row.sxwType"> -->
            ...
            <el-tab-pane label="已审核正确" name="2">
                ...
                >
                    ...
                    <el-table-column prop="sentence" label="所在上下文" width="240">
                        <template slot-scope="scope">
                            <!-- <span v-if="scope.row.sentence" class="sxwClass">
                                <span>{{scope.row.wordLeft}}</span>
                                <span style="color:red;borderBottom:1px solid red">{{scope.row.word}}</span>
                                <span>{{scope.row.wordRight}}</span>
                            </span>
            ...
            <el-tab-pane label="已审核错误" name="3">
                ...
                >
                    ...
                    <el-form-item label="订单批次" prop="">
                        <el-select v-model="batchId" filterable remote placeholder="请选择订单批次" @change="pcChange" :remote-method="remoteMethod2" clearable style="width:310px">
                            <el-option
                            v-for="item in orderBatchList"
                            :key="item.value"
                            :label="item.label"
...
export default {
    ...
    methods: {
        ...
        getList() {
            ...
            listInspSiteInfo(this.queryParams).then(response => {
                ...
                for(var i in response.rows){
                    ...
                    // 列表上下文
                    response.rows[i].wordLeft = response.rows[i].picSentence.split(response.rows[i].word)[0];
                    response.rows[i].wordRight = response.rows[i].picSentence.split(response.rows[i].word)[1];

                    // 所在文章
                    if(response.rows[i].fromTitle == null || response.rows[i].fromTitle == ''){
                        if(response.rows[i].fromUrl == null){
                            response.rows[i].fromTitle = '--';
                        }else{
                            response.rows[i].fromTitle = response.rows[i].fromUrl;
                        }
                    }
                    console.log(response.rows[i].oldwordLeft,'前')
                    console.log(response.rows[i].oldwordRight,'后')
        ...
        // 订单事件
        ddChange(){
            // if(this.queryParams.batchInfo.orderId){
            //     this.queryParams.batchInfo.batchId = null;
            //     this.getorderBatchIdDateList('',this.queryParams.batchInfo.orderId)
            // }else{
            //     this.queryParams.batchInfo.batchId = null;
            //     this.orderBatchList = [];
            //     this.orderInfoList = [];
