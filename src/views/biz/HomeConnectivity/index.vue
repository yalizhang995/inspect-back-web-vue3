<template>
    <div class="app-container">
        ...
        <el-table :data="tableData" stripe style="width: 100%" height="59vh" @sort-change="sortChange" v-loading="loading" fit>
            <el-table-column prop="index" label="序号" min-width="50"  align="center"></el-table-column>
            <el-table-column prop="siteName" label="网站名称" min-width="140" show-overflow-tooltip>
            ...
            <el-table-column prop="siteCode" label="网站标识码" sortable="custom"  ></el-table-column>
            <el-table-column prop="countNum" label="连通总数" align="center" sortable="custom" >
                <template slot-scope="scope">
                    <span class="dz" @click="goInfo(scope.row)">{{ scope.row.countNum}}</span>
                </template>
            </el-table-column>
            <el-table-column prop="normalSum" label="成功连通" align="center" sortable="custom" >
                <template slot-scope="scope">
                    <span class="dz" @click="goInfo(scope.row,'1')">{{ scope.row.normalSum}}</span>
                </template>
            </el-table-column>
            <el-table-column prop="confirmSum" label="确定不连通" align="center" sortable="custom"  >
                <template slot-scope="scope">
                    <span class="dz" @click="goInfo(scope.row,'2')">{{ scope.row.confirmSum}}</span>
                </template>
            </el-table-column>
            <el-table-column prop="suspectedSum" label="疑似不连通" align="center" sortable="custom"  >
                <template slot-scope="scope">
                    <span class="dz" @click="goInfo(scope.row,'3')">{{ scope.row.suspectedSum}}</span>
                </template>
            </el-table-column>
            <el-table-column prop="confirmProportionVal" label="确定不连通占比" align="center" sortable="custom"  >
                <template slot-scope="scope">
                    <span class="dz" @click="goInfo(scope.row,'2')">{{ scope.row.confirmProportionVal}}</span>
                </template>
            </el-table-column>
            <el-table-column prop="suspectedProportionVal" label="疑似不连通占比" align="center" sortable="custom"  >
                <template slot-scope="scope">
                    <span class="dz" @click="goInfo(scope.row,'3')">{{ scope.row.suspectedProportionVal}}</span>
                </template>
            </el-table-column>
            <el-table-column prop="scanDate" label="监测时间"   align=""></el-table-column>
            <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="80" fixed="right">
                <template slot-scope="scope">
                    <el-tooltip class="item" effect="dark" content="详情" placement="top">
                        <i class="el-icon-view" style="marginLeft:10px;cursor: pointer;font-size:15px" @click="goInfo(scope.row)"></i>
                    </el-tooltip>
                </template>
            </el-table-column>
        </el-table>
            ...
            expireTimeOPtion: {
                ...
                disabledDate: (time) => {
                    ...
                    if (start && !end) {
                        ...
                        // 只能选择开始日期前后一年的日期
                        return time.getTime() > oneYearLater.getTime() || 
    ...
    methods: {
       ...
       /** 查询站点关系下拉树结构 */
        ...
        getList() {
            this.loading = true;
            if(this.scanDate){
                this.queryParams.scanStartDate = this.formatDate(this.scanDate[0]);
                this.queryParams.scanEndDate = this.formatDate(this.scanDate[1]);
            }else{
                this.queryParams.scanStartDate = null;
                this.queryParams.scanEndDate = null;
            }
            getList(this.queryParams).then((res)=>{
                ...
                for(var i in this.tableData){
                    ...
                    if(!this.tableData[i].suggest){
                        this.tableData[i].suggest = '--';
                    }
                    // 问题级别状态颜色
                    if(this.tableData[i].severity == 1){ // 严重
                        this.tableData[i].severityBgColor = '#ff6563';
                    }else if(this.tableData[i].severity == 2){ // 一般
                        this.tableData[i].severityBgColor = '#ea9940';
                    }else if(this.tableData[i].severity == 3){ // 轻微
                        this.tableData[i].severityBgColor = '#48b93f';
                    }
                    // 确认不连通占比
                    //   if(this.tableData[i].confirmProportion != 0){ // 严重
                        this.tableData[i].confirmProportion = this.tableData[i].confirmProportion*100+'%';
                    //   }
                    // 疑似不连通占比  
                    //   if(this.tableData[i].suspectedProportion != 0){ // 严重
                        this.tableData[i].suspectedProportion = this.tableData[i].suspectedProportion*100+'%';
                    //   }

                    if(!this.tableData[i].siteName){
                        this.tableData[i].siteName = '--';
                    }
            ...
            this.$refs["form4"].validate(valid => {
                if (valid) {
                    ...
                    if(this.form4.type == 2){ // 大批量
                        ...
                        let fileName = '首页连通数据'
                        this.download(
                            "biz/links/export",this.queryParams,
                            `${fileName}.xlsx`
                        ).then((res)=>{
                            this.open4 = false;
                        });
        ...
        sjrqChange(val){
            ...
            if(val == null){
                ...
                // 如果超过一年，自动调整
                if (diff > oneYearMs) {
                    if (this.adjusting) return; // 防止无限循环
                    
                    this.adjusting = true;
                    const newEndDate = new Date(startDate.getTime() + oneYearMs);
                    this.scanDate = [startDate, newEndDate];
                    
                    this.$nextTick(() => {
                        this.adjusting = false;
                        this.$message.warning('选择范围不能超过一年');
                    });
                }
            }
        },
        ...
        goInfo(val,typeNum){
            console.log(val,'val')
            this.$router.push({path: '/machine/HomeConnectivityInfo'})
            sessionStorage.setItem('HomeConnectivityInfo',JSON.stringify(val))
            if(typeNum){
                sessionStorage.setItem('HomeConnectivityInfoType',typeNum)
            }
        },
