<template>
    <div class="app-container">
        <el-form
            ...
            label-width="110px"
        >
            <el-form-item label="填报单位标识码" prop="parentSite" >
                <el-input
        ...
        >
            ...
            <el-table-column label="填报单位标识码"  prop="parentSite" min-width="140" />
            <el-table-column label="新媒体标识码"  prop="siteCodeMedia" min-width="160" />
            <el-table-column label="原填报单位标识码"  prop="fromSite" min-width="140" />
...
export default {
    ...
    beforeRouteLeave(to, from, next) {
        ...
        console.log('离开页面了！');
        // 确保调用next()以便路由继续跳转
        sessionStorage.removeItem('mediaRecordId')
        next();

    },
    created() {
        if(sessionStorage.getItem('mediaRecordId')){
            this.queryParams.taskId = sessionStorage.getItem('mediaRecordId')
        }
        this.getList();
    },
    methods: {
            ...
            siteTreeImportRecord(this.queryParams).then(response => {
                ...
                for(var i in this.inspImportdataTaskList){
                    ...
                    for(var j in this.actionTypeList){
                        ...
                        }else if(this.inspImportdataTaskList[i].actionType == null){
                            this.inspImportdataTaskList[i].actionTypeTxt = '--';
                        }
                    }
                    if(!this.inspImportdataTaskList[i].siteCodeMedia){
                        this.inspImportdataTaskList[i].siteCodeMedia = '--';
                    }
                    if(!this.inspImportdataTaskList[i].fromSite){
                        this.inspImportdataTaskList[i].fromSite = '--';
                    }
                }
