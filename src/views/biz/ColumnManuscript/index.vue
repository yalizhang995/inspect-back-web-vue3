<template>
    <div class="app-container">
        ...
        >
            ...
            <el-table-column prop="name" label="栏目名称" width="140" align="center" show-overflow-tooltip> 
                <template slot-scope="scope">
                    <span class="dz" @click="dzChange(scope.row.fromUrl)">{{ scope.row.name}}</span>
                </template>
            </el-table-column>
            
            <el-table-column prop="channelTypeText" label="栏目分类" width="200" align="center" show-overflow-tooltip></el-table-column>
            ...
            <el-table-column prop="title" label="稿件标题"  width="400"  show-overflow-tooltip>
                <template slot-scope="scope">
                    <!-- <el-tooltip class="item" effect="dark" :content="scope.row.title" placement="top"> -->
            ...
            <el-table-column prop="fromUrl" label="父URL地址" width="300" show-overflow-tooltip >
                <template slot-scope="scope">
            ...
            <el-table-column prop="pubTime" label="发布时间" width="180" sortable="custom"></el-table-column>
            <el-table-column prop="scanTime" label="发现时间" width="180" sortable="custom"></el-table-column>
        ...
        <el-dialog title="稿件重采" :visible.sync="open10" width="600px" append-to-body :close-on-click-modal="false" :close-on-press-escape="false" @close="cancel10">
            <el-form ref="form10" :model="form10" :rules="rules10" label-width="120px">
                ...
                <el-form-item label="栏目地址" prop="fromUrl">
                    <el-input v-model="form10.fromUrl" placeholder="请输入栏目地址" clearable style="width:90%" />
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
...
export default {
    ...
    beforeRouteLeave(to, from, next) {
        ...
        console.log('离开页面了！');
        // 确保调用next()以便路由继续跳转
        sessionStorage.removeItem('columnUpdateObj')
        sessionStorage.removeItem('columnManuscriptColumnObj')
        next();
    ...
    activated(){
        // 栏目更新跳转
        if(sessionStorage.getItem('columnUpdateObj')){
            let columnUpdateObj = JSON.parse(sessionStorage.getItem('columnUpdateObj'));
            // this.queryParams.siteCode = columnUpdateObj.siteCode;
            // this.queryParams.fromUrl = columnUpdateObj.url;
            this.$set(this.queryParams,'siteCode', columnUpdateObj.siteCode)
            this.$set(this.queryParams,'fromUrl', columnUpdateObj.url)
            if(columnUpdateObj.specialTag == 1){
                this.$set(this.queryParams,'checkDate', columnUpdateObj.renewTime)
            }
            this.scanDate = [];
        ...
        // 爬虫任务跳转
        if(sessionStorage.getItem('columnManuscriptColumnObj')){
            let columnUpdateObj = JSON.parse(sessionStorage.getItem('columnManuscriptColumnObj'));
            // this.queryParams.siteCode = columnUpdateObj.siteCode;
            // this.queryParams.fromUrl = columnUpdateObj.url;
            this.$set(this.queryParams,'siteCode', columnUpdateObj.siteCode)
            this.$set(this.queryParams,'fromUrl', columnUpdateObj.url)
