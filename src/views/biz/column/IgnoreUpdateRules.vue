<template>
    <div class="app-container" id="information">
        ...
        >
            <div class="box">
                <div class="collapse">
                    <div style="color:red;margin: 0 0 10px 20px">注：以标签范围进行忽略</div>
...
export default {
    name: "Column",
    props: {
        getIgnoreUpdateRuless: Boolean,
        getIgnoreUpdateRulesDateIdStr: String,
        getIgnoreUpdateRulesDate: Array,
    },
    data() {
        return {
            // 是否显示弹出层
    ...
    created() {
        console.log(this.getIgnoreUpdateRulesDate,'this.getIgnoreUpdateRulesDate')
        this.open = this.getIgnoreUpdateRuless;
        this.form.id = this.getIgnoreUpdateRulesDateIdStr;
        this.form.task.ignoreUpdateRules = this.getIgnoreUpdateRulesDate;
        
        this.politicalDate = null;
        if(this.form.task.ignoreUpdateRules.length == 0){

        }else{
            this.politicalDate = [];
            for(var i in this.form.task.ignoreUpdateRules){
                this.politicalDate.push({value:this.form.task.ignoreUpdateRules[i]})
            }
        }
        // }
    },
    methods: {
        ...
        submitForm() {
            ...
            if(this.politicalDate != null){
                this.form.task.ignoreUpdateRules = [];
                for(var i in this.politicalDate){
                    // if(this.politicalDate[i].value){
                        this.form.task.ignoreUpdateRules.push(this.politicalDate[i].value)
                    // }
                }
            }else{
                this.form.task.ignoreUpdateRules = null;
            }
