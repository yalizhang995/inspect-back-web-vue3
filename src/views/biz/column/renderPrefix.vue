...
export default {
    name: "Column",
    props: {
        getRenderPrefixs: Boolean,
        getRenderPrefixDateIdStr: String,
        getRenderPrefixDate: Array,
    },
    data() {
        return {
            // 是否显示弹出层
    ...
    created() {
        console.log(this.getRenderPrefixDate,'this.getRenderPrefixDate')
        this.open = this.getRenderPrefixs;
        this.form.id = this.getRenderPrefixDateIdStr;
        this.form.task.renderPrefixList = this.getRenderPrefixDate;
        
        this.politicalDate = null;
        if(this.form.task.renderPrefixList.length == 0){

        }else{
            this.politicalDate = [];
            for(var i in this.form.task.renderPrefixList){
                this.politicalDate.push({value:this.form.task.renderPrefixList[i]})
            }
        }
        // }
    },
    methods: {
        ...
        submitForm() {
            ...
            if(this.politicalDate != null){
                this.form.task.renderPrefixList = [];
                for(var i in this.politicalDate){
                    // if(this.politicalDate[i].value){
                        this.form.task.renderPrefixList.push(this.politicalDate[i].value)
                    // }
                }
            }else{
                this.form.task.renderPrefixList = null;
            }
