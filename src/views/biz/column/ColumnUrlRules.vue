...
export default {
    name: "Column",
    props: {
        getColumnUrlRules: Boolean,
        getColumnUrlRulesDateIdStr: String,
        getColumnUrlRulesDate: Array,
    },
    watch:{
        // 这里监听dialogShow对象的变化
    ...
    created() {
        ...
        console.log(this.getColumnUrlRulesDate,'this.getColumnUrlRulesDate')
        this.open = this.getColumnUrlRules;
        this.form.id = this.getColumnUrlRulesDateIdStr;
        this.form.task.columnUrlRules = this.getColumnUrlRulesDate;
        
        this.politicalDate = null;
        if(this.form.task.columnUrlRules.length == 0){

        }else{
            this.politicalDate = [];
            for(var i in this.form.task.columnUrlRules){
                this.politicalDate.push({value:this.form.task.columnUrlRules[i]})
            }
        }
        // }
    },
    methods: {
        ...
        submitForm() {
            ...
            if(this.politicalDate != null){
                this.form.task.columnUrlRules = [];
                for(var i in this.politicalDate){
                    // if(this.politicalDate[i].value){
                        this.form.task.columnUrlRules.push(this.politicalDate[i].value)
                    // }
                }
            }else{
                this.form.task.columnUrlRules = null;
            }
