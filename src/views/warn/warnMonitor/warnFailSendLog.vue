<template>
    <div class="app-container">
        ...
        >
            ...
            <el-table-column label="触发条件"  prop="" align="center" width="80" v-if="columns[3].visible">
            ...
            <el-table-column label="发送状态" align="center"  prop="sendStatusTxt" width="100" v-if="columns[14].visible">
                <template slot-scope="scope">
                    <span :style="{color: scope.row.sendStatusColor}" :class="scope.row.sendStatus == 4 ? 'activeClass' : ''">{{ scope.row.sendStatusTxt }}</span>
                </template>
            </el-table-column>
            <el-table-column label="创建时间"  prop="createTime" width="160" v-if="columns[15].visible"/>
            <el-table-column label="策略ID"  prop="strategyId" width="80" v-if="columns[16].visible"/>
            <el-table-column label="来源ID"  prop="warnSendLogId" width="210" v-if="columns[17].visible"/>
        </el-table>
...
export default {
    ...
    data() {
        return {
            ...
            columns: [
                ...
                { key: 16, label: `策略ID`, visible: true },
                { key: 17, label: `来源ID`, visible: true },
    ...
    created() {
        if(sessionStorage.getItem('warnFailSendLogId')){
            this.queryParams.warnSendLogId = sessionStorage.getItem('warnFailSendLogId');
            this.getList();
        }else{
            this.getList();
        }
    ...
    beforeUnmount() {
        // 清理缓存的代码
        sessionStorage.removeItem('warnFailSendLogId');
    },
    methods: {
        ...
        getList() {
            this.loading = true;
            getFailLogList(this.queryParams).then(response => {
        ...
        goBack(){
            ...
            this.$store.dispatch('tagsView/delView', view).then(({ visitedViews }) => {
                ...
                // this.$router.go(-1);    
                this.$router.push({path:'/warn/warnSendLog'})
